...
def sales_trend():
    ...
    # 1. 月度趋势：补全 1-12 月数据，确保图表横轴完整
    monthly = df.groupby(df['purchase_time'].dt.month).agg(
        sales=('amount', 'sum'),
        orders=('user_id', 'count')
    ).reindex(range(1, 13), fill_value=0).reset_index()
    
    monthly.columns = ['month_num', 'sales', 'orders']
    monthly['month'] = monthly['month_num'].apply(lambda x: f"{x}月")
    # 计算月平均客单价，避免除以零错误
    monthly['avgAmount'] = np.where(monthly['orders'] > 0, monthly['sales'] / monthly['orders'], 0.0)
    monthly_data = monthly[['month', 'sales', 'orders', 'avgAmount']].to_dict('records')
    ...
    curr_sales = monthly[monthly['month_num'] == curr_month]['sales'].values[0] if curr_month in monthly['month_num'].values else 0.0
    prev_sales = monthly[monthly['month_num'] == last_month]['sales'].values[0] if last_month in monthly['month_num'].values else 1.0 
